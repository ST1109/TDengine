...
# option to insert testdata, default is no
TESTDATA_INSERT=${INSERT_TESTDATA:-0}
unset INSERT_TESTDATA
...
if [ -f "$DATA_DIR/dnode/dnodeEps.json" ]; then
    CURRENT_FQDN=`hostname`
    if [ "$TAOS_FQDN" != "" ]; then
        CURRENT_FQDN="$TAOS_FQDN"
    fi

    sed -i "s/\"dnodeFqdn\": .*$/\"dnodeFqdn\": \"$CURRENT_FQDN\",/" $DATA_DIR/dnode/dnodeEps.json || echo -e "failed to replace FQDN"
fi

if [ "$TESTDATA_INSERT" != "0" ]; then
    taosd &
    sleep 5
    taos -s "drop database if exists test"
    taosBenchmark -S 900000 -I stmt -y
    pkill taosd
fi
